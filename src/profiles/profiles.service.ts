...
import {
  PatientInformation,
  PatientInformationDocument,
} from './schemas/patient_information.schema';
import {
  PharmacyProfile,
...
import {
  LabProfile,
  LabProfileDocument,
} from '../lab/schemas/lab-profile.schema';
import {
  ClinicProfile,
  ClinicProfileDocument,
...
export class ProfilesService {
  constructor(
    ...
    @InjectModel(PatientInformation.name)
    private patientModel: Model<PatientInformationDocument>,
    ...
    @InjectModel(ClinicProfile.name)
    private clinicModel: Model<ClinicProfileDocument>,
  ...
  async getProfile(userId: string, role: UserRole): Promise<any> {
    ...
    console.log(
      `[ProfilesService] Fetching profile for userId: ${userId}, role: ${roleLower}`,
    );
    ...
    switch (roleLower) {
      ...
      case UserRole.PATIENT.toString():
        const patientProfile = await this.patientModel
          .findOne({ userId: objectId })
          .exec();
        console.log(
          `[ProfilesService] Patient profile found: ${!!patientProfile}`,
        );
  ...
  async upsertDoctorProfile(
    ...
    if (!userId || userId === 'undefined') {
      throw new BadRequestException(
        'ID utilisateur manquant pour la mise à jour du profil médecin',
      );
    }
    const objectId = new Types.ObjectId(userId);
  ...
  async upsertPatientInformation(
    userId: string,
    data: Partial<PatientInformation>,
  ): Promise<PatientInformationDocument> {
    if (!userId || userId === 'undefined') {
      throw new BadRequestException(
        'ID utilisateur manquant pour la mise à jour des informations',
      );
    }
    const objectId = new Types.ObjectId(userId);

    console.log(
      `[ProfilesService] Upserting PatientInformation for userId: ${userId}`,
    );
    ...
    console.log(
      `[ProfilesService] PatientInformation upserted: ${profile._id}`,
    );
  ...
  async upsertPharmacyProfile(
    userId: string,
    data: Partial<PharmacyProfile>,
  ): Promise<PharmacyProfileDocument> {
    if (!userId || userId === 'undefined') {
      throw new BadRequestException(
        'ID utilisateur manquant pour la mise à jour du profil pharmacie',
      );
    }
    const objectId = new Types.ObjectId(userId);

    try {
      const profile = await this.pharmacyModel
        .findOneAndUpdate(
          { userId: objectId },
          { ...data, userId: objectId },
  ...
  async upsertLabProfile(
    userId: string,
    data: Partial<LabProfile>,
  ): Promise<LabProfileDocument> {
    if (!userId || userId === 'undefined') {
      throw new BadRequestException(
        'ID utilisateur manquant pour la mise à jour du profil laboratoire',
      );
    }
    const objectId = new Types.ObjectId(userId);
    ...
    const profile = await this.labModel
      .findOneAndUpdate(
        { userId: objectId },
  ...
  async upsertClinicProfile(
    userId: string,
    data: Partial<ClinicProfile>,
  ): Promise<ClinicProfileDocument> {
    if (!userId || userId === 'undefined') {
      throw new BadRequestException(
        'ID utilisateur manquant pour la mise à jour du profil clinique',
      );
    }
    const objectId = new Types.ObjectId(userId);

    const profile = await this.clinicModel
      .findOneAndUpdate(
        { userId: objectId },
        { ...data, userId: objectId },
        { upsert: true, new: true },
